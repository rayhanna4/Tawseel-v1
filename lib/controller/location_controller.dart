...
import 'package:qareeb/data/model/response/place_details_model.dart';
import 'package:qareeb/data/model/response/prediction_model.dart';
...
import 'package:qareeb/view/base/confirmation_dialog.dart';
...
import 'package:qareeb/view/screens/location/widget/module_dialog.dart';
import 'package:qareeb/view/screens/location/widget/permission_dialog.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  Future<ZoneResponseModel> getZone(String lat, String long, bool markerLoad,[int id]) async {
    if(markerLoad) {
      _loading = true;
    }else {
      _isLoading = true;
    ...
    Response response = await locationRepo.getZone(lat, long,id);
  ...
  void saveAddressAndNavigate(
      AddressModel address, bool fromSignUp, String route, bool canRoute, bool isDesktop,
      {bool fromPick = false,int id}) {
    if(Get.find<CartController>().cartList.length > 0) {
      Get.dialog(ConfirmationDialog(
        ...
        onYesPressed: () {
          Get.back();
          //hamza
          _setZoneData(address, fromSignUp, route, canRoute, isDesktop,fromPick: fromPick,id: id);
  ...
  void _setZoneData(AddressModel address, bool fromSignUp, String route, bool canRoute, bool isDesktop,
      {bool fromPick = false,int id}) {
    Get.find<LocationController>().getZone(address.latitude, address.longitude, false,id).then((response) async {
      if (response.isSuccess) {
        print('hamza6');
        print(response.zoneIds);
